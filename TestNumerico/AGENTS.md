...
- Crea la clase de la acción en el paquete `ni.uam.edu.TestNumerico.acciones`, normalmente extendiendo `ViewBaseAction`
...
<controlador nombre="TuPrimeraEntidad">
    ...
    <accion nombre="mostrarImporte" modo="detail"
        clase="ni.uam.edu.TestNumerico.acciones.MostrarImporte"/>
</controlador>
```

```java
package ni.uam.edu.TestNumerico.acciones;
...
public class MostrarImporte extends ViewBaseAction {
    ...
    public void execute() throws Exception {
        ...
        if (importe == null) {
            addError("importe_no_establecido"); // Añade importe_no_establecido al fichero de mensajes i18n
        } else {
            addMessage("importe_mensaje", importe); // Añade importe_mensaje al fichero de mensajes i18n
        }
    }
}
```
...
- ✅ Coloca las clases de acción en `ni.uam.edu.TestNumerico.acciones`
...
```java
package ni.uam.edu.TestNumerico.pruebas;
...
- Crea un paquete `cuadrosmando` (ej. `ni.uam.edu.TestNumerico.cuadrosmando`) y coloca la clase `CuadroMando` y sus clases auxiliares en él.
